...
class AccountMove(models.Model):
    ...
    consignatario_fel = fields.Many2one(
        "res.partner", string="Consignatario o Destinatario FEL"
    )
    ...
    documento_xml_fel_name = fields.Char(
        "Nombre doc xml FEL", default="documento_xml_fel.xml", size=32
    )
    resultado_xml_fel = fields.Binary("Resultado xml FEL", copy=False)
    resultado_xml_fel_name = fields.Char(
        "Resultado doc xml FEL", default="resultado_xml_fel.xml", size=32
    )
    ...
    def error_certificador(self, error):
        ...
        if factura.journal_id.error_en_historial_fel:
            factura.message_post(
                body="<p>No se publicó la factura por error del certificador FEL:</p> <p><strong>"
                + error
                + "</strong></p>"
            )
        else:
            raise UserError(
                "No se publicó la factura por error del certificador FEL: " + error
            )
    ...
    def requiere_certificacion(self, certificador=""):
        ...
        requiere = (
            factura.is_invoice()
            and factura.journal_id.generar_fel
            and factura.amount_total != 0
        )
        if certificador:
            requiere = requiere and (
                factura.company_id.certificador_fel == certificador
            )
        return requiere
    ...
    def error_pre_validacion(self):
        ...
        if factura.firma_fel:
            factura.error_certificador(
                "La factura ya fue validada, por lo que no puede ser validada nuevamnte"
            )
            return True
        ...
        if precio_total_descuento > 0:
            for linea in factura.invoice_line_ids:
                if linea.price_unit > 0:
                    descuento = (
                        precio_total_descuento / precio_total_positivo
                    ) * 100 + linea.discount
                    name = linea.name
                    factura.write(
                        {"invoice_line_ids": [[1, linea.id, {"discount": descuento}]]}
                    )
    ...
    def dte_documento(self):
        ...
        GTDocumento = etree.Element(
            DTE_NS + "GTDocumento", {}, Version="0.1", nsmap=NSMAP
        )
        ...
        tipo_interno_factura = (
            factura.type if "type" in factura.fields_get() else factura.move_type
        )
        if (
            tipo_documento_fel in ["FACT", "FACM"]
            and tipo_interno_factura == "out_refund"
        ):
        ...
        fecha = (
            factura.invoice_date.strftime("%Y-%m-%d")
            if factura.invoice_date
            else fields.Date.context_today(self).strftime("%Y-%m-%d")
        )
        hora = datetime.now().strftime("%H:%M:00-06:00")
        fecha_hora = fecha + "T" + hora
        logging.warning("FECHA %s", fecha_hora)
        ...
        if len(factura.journal_id.direccion.street) == 0:
            Direccion.text = "CIUDAD"
        else:
        ...
        Departamento.text = (
            factura.journal_id.direccion.state_id.name
            if factura.journal_id.direccion.state_id
            else ""
        )
        ...
        Receptor = etree.SubElement(
            ...
            if not factura.partner_id.parent_id
            else factura.partner_id.parent_id.name,
        )
        if factura.partner_id.nombre_facturacion_fel:
            Receptor.attrib[
                "NombreReceptor"
            ] = factura.partner_id.nombre_facturacion_fel
        ...
        if not factura.partner_id.street:
            Direccion.text = factura.partner_id.city or "CIUDAD"
        else:
            Direccion.text = (
                (factura.partner_id.street or "")
                + " "
        ...
        Departamento.text = (
            factura.partner_id.state_id.name if factura.partner_id.state_id else ""
        )
        ...
        for linea in factura.invoice_line_ids:
            ...
            descuento = (
                precio_sin_descuento * linea.quantity - precio_unitario * linea.quantity
            )
            ...
            Cantidad.text = "{:.2f}".format(linea.quantity)
            UnidadMedida = etree.SubElement(Item, DTE_NS + "UnidadMedida")
            UnidadMedida.text = (
                linea.product_uom_id.name[0:3] if linea.product_uom_id else "UNI"
            )
        ...
        if factura.company_id.adenda_fel:
            ...
            exec(
                factura.company_id.adenda_fel,
                {"etree": etree, "Adenda": Adenda, "factura": factura},
            )

        # En todos estos casos, es necesario enviar complementos
        if (
            tipo_documento_fel in ["NDEB", "NCRE"]
            or tipo_documento_fel in ["FCAM"]
            or (
                tipo_documento_fel in ["FACT", "FCAM"]
                and factura.tipo_gasto == "importacion"
            )
            or tipo_documento_fel in ["FESP"]
        ):
            ...
            if (
                ...
                Exportacion = etree.SubElement(
                    Complemento, CEX_NS + "Exportacion", Version="1", nsmap=NSMAP_EXP
                )
                NombreConsignatarioODestinatario = etree.SubElement(
                    Exportacion, CEX_NS + "NombreConsignatarioODestinatario"
                )
                NombreConsignatarioODestinatario.text = (
                    factura.consignatario_fel.name if factura.consignatario_fel else "-"
                )
                DireccionConsignatarioODestinatario = etree.SubElement(
                    Exportacion, CEX_NS + "DireccionConsignatarioODestinatario"
                )
                DireccionConsignatarioODestinatario.text = (
                    factura.consignatario_fel.street or "-"
                    if factura.consignatario_fel
                    else "-"
                )
                CodigoConsignatarioODestinatario = etree.SubElement(
                    Exportacion, CEX_NS + "CodigoConsignatarioODestinatario"
                )
                CodigoConsignatarioODestinatario.text = (
                    factura.consignatario_fel.ref or "-"
                    if factura.consignatario_fel
                    else "-"
                )
                ...
                NombreComprador.text = (
                    factura.comprador_fel.name if factura.comprador_fel else "-"
                )
                DireccionComprador = etree.SubElement(
                    Exportacion, CEX_NS + "DireccionComprador"
                )
                DireccionComprador.text = (
                    factura.comprador_fel.street or "-"
                    if factura.comprador_fel
                    else "-"
                )
                CodigoComprador = etree.SubElement(
                    Exportacion, CEX_NS + "CodigoComprador"
                )
                CodigoComprador.text = (
                    factura.comprador_fel.ref or "-" if factura.comprador_fel else "-"
                )
                ...
                NombreExportador.text = (
                    factura.exportador_fel.name if factura.exportador_fel else "-"
                )
                CodigoExportador = etree.SubElement(
                    Exportacion, CEX_NS + "CodigoExportador"
                )
                CodigoExportador.text = (
                    factura.exportador_fel.ref or "-" if factura.exportador_fel else "-"
                )
            ...
            if tipo_documento_fel in ["FESP"]:
                ...
                RetencionISR = etree.SubElement(
                    RetencionesFacturaEspecial, CFE_NS + "RetencionISR"
                )
                RetencionISR.text = str(total_isr)
                RetencionIVA = etree.SubElement(
                    RetencionesFacturaEspecial, CFE_NS + "RetencionIVA"
                )
                RetencionIVA.text = str(total_iva_retencion)
                TotalMenosRetenciones = etree.SubElement(
                    RetencionesFacturaEspecial, CFE_NS + "TotalMenosRetenciones"
                )
    ...
    def dte_anulacion(self):
        ...
        fecha_hoy_hora = datetime.now().strftime(
            "%Y-%m-%dT%H:%M:%S"
        )

        GTAnulacionDocumento = etree.Element(
            DTE_NS + "GTAnulacionDocumento", {}, Version="0.1", nsmap=NSMAP
        )
        SAT = etree.SubElement(GTAnulacionDocumento, DTE_NS + "SAT")
        AnulacionDTE = etree.SubElement(
            SAT, DTE_NS + "AnulacionDTE", ID="DatosCertificados"
        )
        logging.warning("FECHA HORA FACTURA", fecha_hora)
        logging.warning("FECHA HORA ANULACION", fecha_hoy_hora)
...
class AccountJournal(models.Model):
    ...
    error_en_historial_fel = fields.Boolean(
        "Registrar error FEL",
        help="Los errores no se muestran en pantalla, solo se registran en el historial",
    )
